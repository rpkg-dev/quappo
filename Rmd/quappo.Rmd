...
view_output <- function(input = ".",
  ...
  if (output_format == "html") {
    ...
    if (!fs::file_exists(fs::path(output_dir, "index.html"))) {
      cli::cli_abort(paste0("No {.file index.html} file found under {.path {input}}. Please first render the specified Quarto project ",
    ...
    is_port_available <- utils::getFromNamespace(x = "is_port_available",
                                                 ns = "httpuv")
    paths_static <- list("/" = fs::path_abs(input))
    ...
    target <-
      ...
      stringr::str_replace_all(string = config$book$title,
                               ...
                               replacement = "-") |>
      pal::when(length(.) == 0L,
                fs::path_file(output_dir)) |>
      fs::path(ext = output_format) |>
      fs::path_abs(start = output_dir)
    
    if (!fs::file_exists(target)) {
      cli::cli_abort(paste0("No output file found under {.path {output_dir}}. Please first render the specified Quarto project to {.val output_format} (e.g. ",
