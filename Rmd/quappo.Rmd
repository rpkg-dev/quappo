...
-   We can't retrieve the set of valid `output_format`s from a project's Quarto config since output formats can be valid without being explicitly configured.
...
-   Submit upstream PR to adapt `quarto::quarto_render()` to check for `is.null()` instead of
    [`missing()`](https://github.com/quarto-dev/quarto-r/blob/0fcb998247fb8f33d1b54b6b8c7841ed473300e9/R/render.R) to be able to properly forward all possibly
    relevant args.
...
#'
#' @details
#' `r pkgsnip::md_snip("rstudio_addin")` Note that the default `path = rstudioapi::getActiveDocumentContext()$path` does only work when called via a
...
#' @param path Path to a file (["chapter"](https://quarto.org/docs/books/book-structure.html#overview)) or subdirectory
#'   (["part"](https://quarto.org/docs/books/book-structure.html#parts-appendices)) of a [Quarto book](https://quarto.org/docs/reference/projects/books.html)
#'   project.
#'
...
render_partly <- function(path = rstudioapi::getActiveDocumentContext()$path,
                          ...
                          quiet = FALSE,
                          pandoc_args = NULL,
                          as_job = getOption("quarto.render_as_job",
                                             default = interactive() && rstudioapi::isAvailable())) {
  if (!is.null(profile)) {
    profile <- rlang::arg_match(profile,
                                values = profiles(),
                                multiple = TRUE)
  }
  ...
  quarto::quarto_render(input = path,
                        ...
                        execute = execute,
                        execute_daemon_restart = execute_daemon_restart,
                        execute_debug = execute_debug,
                        use_freezer = use_freezer,
...
#'   character scalar that starts with `"fig-"` and only contains letters, numbers, underscores (`_`) and dashes (`-`).
#' @param fig_cap Figure caption. Set as Quarto's [`fig-cap`](https://quarto.org/docs/reference/cells/cells-knitr.html#figures) code chunk option. Either `NULL`
...
#'   [`fig-width`](https://quarto.org/docs/reference/cells/cells-knitr.html#figures) code chunk option. Either `NULL` to omit or a numeric scalar.
#' @param fig_height Height of the plot (in inches), to be used in the graphics device. Set as Quarto's
#'   [`fig-height`](https://quarto.org/docs/reference/cells/cells-knitr.html#figures) code chunk option. Either `NULL` to omit or a numeric scalar.
...
#'   Quarto's [`fig-pos`](https://quarto.org/docs/reference/cells/cells-knitr.html#figures) code chunk option. Either `NULL` to omit, a character scalar, or
#'   `FALSE` for no figure position specifier, which is sometimes necessary with custom figure environments (such as `sidewaysfigure`).
...
#'   option. Either `NULL` to omit or a character scalar.
...
#'   document. Set as Quarto's [`out-height`](https://quarto.org/docs/reference/cells/cells-knitr.html#figures) code chunk option. Either `NULL` to omit or a
#'   character scalar.
#'
...
#'   character scalar that starts with `"tbl-"` and only contains letters, numbers, underscores (`_`) and dashes (`-`).
#' @param tbl_cap Table caption. Set as Quarto's [`tbl-cap`](https://quarto.org/docs/reference/cells/cells-knitr.html#tables) code chunk option. Either `NULL`
#'   to omit or a character vector.
...
collapse_authors <- function(config = quarto::quarto_inspect()$config,
  ...
  config$book$author |>
    ...
    purrr::pmap(\(...) {
      ...
      args <- rlang::list2(...)
      result <- purrr::chuck(args,
                             "name")
      
      if (reverse_first && args$rowid == 1L) {
      ...
      if (length(args$orcid) > 0L && !is.na(args$orcid)) {
        result <- switch(EXPR = orcid_style,
                         none = result,
                         icon = glue::glue("{result} <a href='https://orcid.org/{args$orcid}' target='_blank' rel='noopener'>{orcid_icon}</a>"),
                         plain = glue::glue("{result} (ORCID: {args$orcid})"))
      }
...
#'   quappo::profiles()
#' }
profiles <- function(input = ".") {
  
  fs::dir_ls(path = input,
             type = "file",
             regexp = "(^|/)_quarto-.+\\.yml$") |>
  fs::path_file() |>
...
default_profiles <- function(input = ".") {
  ...
  profile_config <-
    fs::path(input, "_quarto.yml") |>
    # NOTE: we avoid default YAML sequence simplification, cf. https://github.com/vubiostat/r-yaml/issues/69
    yaml::read_yaml(handlers = list(seq = \(x) x)) |>
    purrr::pluck("profile")
  
  # 1st prio: `profile.default`
  # 2nd prio: first `profile.group`
  result <- profile_config$default %||% purrr::map_chr(profile_config$group,
                                                       dplyr::first)
  if (length(result) == 0L) {
    result <- NULL
  }
...
-   `yaml::as.yaml()` takes care of properly [escaping special chrs](https://stackoverflow.com/a/22235064/7196903) in chr vctrs.
